...
- 首次使用流程调整：登录页先呈现隐私授权（含「不同意并退出」，拒绝后进入说明页），同意后再设置 Web 访问密码；密码设置页不再出现任何同意性文字
- 隐私政策补充 Web 访问密码的收集说明（PBKDF2 哈希本地保存、用于登录验证），并按部署形态澄清 Wi-Fi 信息的收集范围（商店版不收集）；协议版本更新至 2026-09-22
- 自动同步默认关闭：首次使用需在 WebUI 中手动开启（已手动设置过的用户保持原有选择）；`config.json` 示例与应用商店内置配置同步调整为默认关闭
- 修复打开本地媒体库时的闪烁与卡顿：视频/LRV 缩略图抽帧全局串行化、同文件去重并限制 ffmpeg 单线程，界面在缩略图生成期间保持流畅
- 界面渲染减负：移除大面积毛玻璃 backdrop-filter（GPU 合成压力是多标签下白屏闪烁的常见诱因），改用不透明度更高的纯色面板，观感基本一致；缩略图改为解码完成后淡入；后台标签页自动暂停状态轮询
- 修复自动同步与日志写入之间的并发死锁
