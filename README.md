...
- 支持 Live 图与 360 全景照片预览：LIV_ 前缀 JPG 自动拆分静帧与动态片段，等距柱状全景图（含相机直出的 JPG 全景）进入球面环视查看器，普通照片不受影响
- 相机文件列表支持点击表头排序（文件名/来源/类型/拍摄日期/大小），默认按拍摄日期从新到旧
...
- UPK 包（绿联应用商店形态）：8766 端口仅容器网络内部可达；8767 应用入口为 TLS 透传（端到端加密），明文 HTTP 请求 301 跳转 HTTPS 以兼容 UGOS 应用中心的 http 打开入口，明文链路不承载任何数据
- UGOS 商店包连接面板替换为系统指引卡片（在 UGOS Pro 的 Wi-Fi 管理界面连接相机热点，应用自动检测相机可达状态）；Docker 部署保持应用内 Wi-Fi 管理不变
